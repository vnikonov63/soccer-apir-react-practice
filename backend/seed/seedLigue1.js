...
import { fetchReverseGeolocationEngland } from "../externalAPIRequests/getHumanReadableAdress.js";
...
const API_GET_SEASON_BUNDESLIGA = `https://soccer.sportmonks.com/api/v2.0/leagues/301?api_token=${process.env.SPORTMONKS_API_CODE}`;
  ...
  const getLocationsName = async () => {
    return Promise.all(
      teamsFiltered.map((team) => {
        try {
          console.log(team.VenueLocation);
          return fetchReverseGeolocationEngland(team.VenueLocation);
        } catch (error) {
          console.error(error);
          console.log(team);
        }
